...
class PeaklistSelection(BaseWidget):
    def __init__(self, parent, gui_settings=None, footer=None):
    ...
    def load_variables(self):
        ...
        self.update_condition_boxes(2, 'y',
                                    len(self.variables["conditions"]["y"]))
        self.update_condition_boxes(1, 'z',
                                    len(self.variables["conditions"]["z"]))
    ...
    def initUI(self):
        ...
        num_points_label.setSizePolicy(
        ...
        self.x_combobox.valueChanged.connect(
            partial(self.update_condition_boxes, 3, 'x'))
        self.y_combobox.valueChanged.connect(
            partial(self.update_condition_boxes, 2, 'y'))
        self.z_combobox.valueChanged.connect(
            partial(self.update_condition_boxes, 1, 'z'))
        ...
        self.showTreeButton.setSizePolicy(
            QSizePolicy.MinimumExpanding, QSizePolicy.Maximum)
        self.widget3.layout().addWidget(self.peak_list_area, 3, 0, 1, 2)
        self.showTreeButton.clicked.connect(self.peak_list_area.update_tree)
        self.showTreeButton.clicked.connect(self.gui_parent.set_data_sets)
        self.peak_list_area.setSizePolicy(
    ...
    def update_condition_boxes(self, row, dim, value):
        ...
        self.x = self.x_combobox.value()
        self.y = self.y_combobox.value()
        self.z = self.z_combobox.value()
        ...
        if len(valuesDict[dim]) < value:
            [valuesDict[dim].append('')
             for x in range(value - len(valuesDict[dim]))]
        if len(valuesDict[dim]) > value:
            valuesDict[dim] = valuesDict[dim][:value]

        for x in range(value):
            text_box = ValueField(self, x, dim, valuesDict)
            text_box.setSizePolicy(QSizePolicy.Expanding,
