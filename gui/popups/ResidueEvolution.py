...
class ResidueEvolutionPopup(BasePopup):
    ...
    def __init__(self, parent=None, **kw):
        ...
        self.res_evo_cols = LabelledSpinBox(
                                            self,
                                            text="Columns Per Page",
                                            minimum=1,
                                            step=1
                                            )
        self.res_evo_rows = LabelledSpinBox(
                                            self,
                                            text="Rows Per Page",
                                            minimum=1,
                                            step=1
                                            )
        self.res_evo_set_x_values = LabelledCheckbox(
                                                   self,
                                                   "Use User Defined X Values?"
                                                    )
        self.res_evo_x_ticks_nbins = LabelledSpinBox(
                                                     self,
        ...
        self.res_evo_plot_line_style = LabelledCombobox(
                                                        self,
        ...
        self.res_evo_plot_line_width = LabelledSpinBox(
                                                       self,
                                                       "Line Width",
                                                       minimum=0,
                                                       step=1
        ...
        self.res_evo_plot_marker_style = LabelledCombobox(
                                                          self,
                                                          text="Marker Style",
                                                          items=[
                                                              "-",
        ...
        self.res_evo_plot_marker_size = LabelledSpinBox(self,
                                                        "Marker Size",
                                                        minimum=0,
                                                        step=1
                                                        )
        self.res_evo_marker_color = ColourBox(self, text="Marker Colour")
        self.res_evo_plot_fill_between = LabelledCheckbox(
                                                        self,
                                                        text="Draw Data Shade"
                                                         )
        self.res_evo_plot_fill_colour = ColourBox(self, text="Shade Colour")
        self.res_evo_fill_alpha = LabelledDoubleSpinBox(
                                                     self,
                                                     text="Shade Transparency",
                                                     minimum=0,
                                                     maximum=1,
                                                     step=0.1
                                                        )
        self.res_evo_fit_line_colour = ColourBox(self, text="Fit Line Colour")
        self.res_evo_fit_line_width = LabelledSpinBox(
                                                      self,
                                                      text="Fit Line Width",
                                                      minimum=0,
                                                      step=1
                                                      )
        self.res_evo_fit_line_style = LabelledCombobox(
                                                       self,
                                                       text="Fit Line Style",
                                                       items=[
                                                              "-",
    ...
    def set_values(self):
        ...
        self.local_variables["set_x_values"] = \
            self.res_evo_set_x_values.isChecked()
        self.local_variables["x_ticks_nbins"] = \
            self.res_evo_x_ticks_nbins.field.value()
        self.local_variables["x_label"] = self.res_evo_x_label.field.text()
        self.local_variables["line_style"] = \
            self.res_evo_plot_line_style.fields.currentText()
        self.local_variables["line_width"] = \
            self.res_evo_plot_line_width.field.value()
        self.local_variables["line_color"] = \
            self.res_evo_line_color.fields.currentText()
        self.local_variables["marker_color"] = \
            self.res_evo_marker_color.fields.currentText()
        self.local_variables["marker_style"] = \
            self.res_evo_plot_marker_style.fields.currentText()
        self.local_variables["marker_size"] = \
            self.res_evo_plot_marker_size.field.value()
        self.local_variables["fill_between"] = \
            self.res_evo_plot_fill_between.isChecked()
        self.local_variables["fill_color"] = \
            self.res_evo_plot_fill_colour.fields.currentText()
        self.local_variables["fill_alpha"] = \
            self.res_evo_fill_alpha.field.value()
        self.local_variables["fit_line_color"] = \
            self.res_evo_fit_line_colour.fields.currentText()
        self.local_variables["fit_line_width"] = \
            self.res_evo_fit_line_width.field.value()
        self.local_variables["fit_line_style"] = \
            self.res_evo_fit_line_style.fields.currentText()
        self.accept()

    def get_values(self):
        self.res_evo_cols.setValue(self.local_variables["cols_page"])
        self.res_evo_rows.setValue(self.local_variables["rows_page"])
        self.res_evo_set_x_values.setChecked(
            self.local_variables["set_x_values"])
        self.res_evo_x_ticks_nbins.setValue(
            self.local_variables["x_ticks_nbins"])
